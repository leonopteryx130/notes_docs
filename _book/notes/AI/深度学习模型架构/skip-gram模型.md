...
Skip-gram 本质上是一个**单层的全连接神经网络**（见 [全连接神经网络](../../数学/应用数学/全连接神经网络.md)）：只有「输入 → 隐藏层 → 输出层」三段，中间没有更深的隐藏层。整体呈**瓶颈结构**——输入、输出都是 $$V$$ 维（词表大小），隐藏层压到 $$N$$ 维（词向量维度，通常 $$N \ll V$$），结构如下图所示。
...
对单条样本，真实标签为 one-hot $$\mathbf{y}$$，预测为 $$\hat{\mathbf{p}}$$，损失为交叉熵（与 [逻辑回归模型的 loss 函数](../../数学/应用数学/逻辑回归模型的loss函数.md) 中多分类形式一致）：
...
会。窗口里有 $$C$$ 个上下文词时（例如窗口大小为 2，左右各 2 个，则 $$C = 4$$），结构上是**一个中心词向量 $$\mathbf{h}_i$$，经同一套 $$W'$$ 去预测 $$C$$ 个标签**。

因为 $$W'$$ 共享，各路的梯度会一起回传到同一个 $$\mathbf{h}_i$$（以及同一个 $$W'$$），所以多个标签之间确实会「互相拉扯」。

这种影响是**好事**：它是一种「一对多」的强约束——中心词的向量必须同时对多个不同的上下文词负责。每次遇到一个中心词，它就会被窗口里的多个上下文「拉扯」多次，等价于用更多监督信号更新该词向量，训练效率更高。

这也是 Skip-gram 在**低频词 / 罕见词**上往往优于 CBOW 的重要原因：罕见词作为中心词出现次数少，但每次出现都能一次性对多个上下文样本产生更新；而 CBOW 是上下文平均后预测中心词，罕见词作为标签被更新的机会更少，学得更慢。
